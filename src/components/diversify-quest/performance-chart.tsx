...
export function PerformanceChart() {
  ...
  // Prepare chart data
  const chartData = gameState.quarterHistory.map((quarter, index) => ({
    ...
    benchmark: 1000000 * Math.pow(1.03, index) / 100000 // 3% quarterly growth benchmark
  }))
  ...
  const currentValue = gameState.portfolio.reduce((sum, holding) => sum + holding.value, 0)
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="hidden md:flex items-center gap-2">
                <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200">₹{(chartData.at(-1)?.portfolioValue ?? 0).toFixed(1)}L</span>
              ...
              <div className="space-y-2 text-sm rounded-lg border border-gray-200 bg-white/60 p-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-emerald-600">
                    +{Math.max(...chartData.map(d => d.quarterReturn), 0).toFixed(2)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Worst Quarter:</span>
                  <span className="font-medium text-red-600">
                    {Math.min(...chartData.map(d => d.quarterReturn), 0).toFixed(2)}%
                  </span>
                </div>
              </div>
            </div>
